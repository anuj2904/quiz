<?php
	...
	try {
		...
		$gen = $_GET['sx'];
		$em = $_GET['em'];
		$sql = "select count(*) 're' from users where userid = '".$uid."'";
		//echo $sql;
		$rs = $conn->query($sql);
		...
		echo'<script type="text/javascript">
			...
			function usercheck(){
				window.location="usercheck.php?unm="+document.qfrm.unm.value+"&nm="+document.qfrm.nm.value+"&sx="+document.qfrm.sx.value+"&em="+document.qfrm.em.value;
			}
		</script>
		...
		if($c==1){
			echo'<body>
				 ...
				 <form method="POST" name="qfrm" action="register.php">
